...
public class LoadWorldObjLoader extends AsynchronousAssetLoader<LoadWorldObjDB, LoadWorldObjLoader.LoadWorldObjDBParameter> {
    private LoadWorldObjDB objDb = new LoadWorldObjDB();

    public LoadWorldObjLoader(FileHandleResolver resolver) {
        super(resolver);
    }

    @Override
    public void loadAsync(AssetManager asman, String filename, FileHandle file, LoadWorldObjDBParameter parameter) {
        ...
        for (int i = 0; i < root.getChildCount(); i++) {
            ...
            XmlReader.Element size = obj.getChild(1);
            ...
            XmlReader.Element tilesElement = obj.getChild(2);
            ...
            for (int k = 0; k < tilesElement.getChildCount(); k++) {
                XmlReader.Element tileElement = tilesElement.getChild(k);
                int x = tileElement.getInt("x");
                int y = tileElement.getInt("y");
                tiles[k] = new GridPoint2(x,y);
            }

            LoadWorldObj justLoaded = new LoadWorldObj(imageNameString, sizeX, sizeY, tiles);
            objDb.addObject(name, justLoaded);
        }
    }

    @Override
    public LoadWorldObjDB loadSync(AssetManager arg0, String arg1, FileHandle arg2, LoadWorldObjDBParameter arg3) {
        return objDb;
    }

    @Override
    @SuppressWarnings("unchecked")
    public Array<AssetDescriptor> getDependencies(String filename, FileHandle file, LoadWorldObjDBParameter parameter) {
        Array<AssetDescriptor> ad = new Array<AssetDescriptor>();
        return ad;
    }

    static public class LoadWorldObjDBParameter extends AssetLoaderParameters<LoadWorldObjDB> {
    }
}
